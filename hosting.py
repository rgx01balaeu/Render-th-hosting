...
# --- Configuration ---
TOKEN = '8516874660:AAEdW3pGjAWLs3Vq44TPejeiPLoEByY-I28'
OWNER_ID = 8306405497
ADMIN_ID = 8306405497
YOUR_USERNAME = '@RGX_WONER'
UPDATE_CHANNEL = 'https://t.me/RGX_TEAM'
# Folder setup
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj, attempt=1):
    ...
    try:
        ...
        terminal_msg = f"""
╔══════════════════════════════════════╗
║      🚀 <b>ＲＧＸ ＨＯＳＴＩＮＧ: STARTING SCRIPT</b> 🚀 ║
╠══════════════════════════════════════╣
...
"""
        ...
        if process.poll() is None:
            success_msg = f"""
╔══════════════════════════════════════╗
║     ✅ <b>ＲＧＸ ＨＯＳＴＩＮＧ: SCRIPT RUNNING</b> ✅   ║
╠══════════════════════════════════════╣
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj, attempt=1):
    ...
    try:
        ...
        terminal_msg = f"""
╔══════════════════════════════════════╗
║      🟢 <b>ＲＧＸ ＨＯＳＴＩＮＧ: STARTING NODE.JS</b> 🟢║
╠══════════════════════════════════════╣
║ 📄 File: <code>{file_name[:25]}</code>
║ 👤 User: {script_owner_id}
║ 🔄 Attempt: {attempt}/{max_attempts}
...
"""
        ...
        if process.poll() is None:
            success_msg = f"""
╔══════════════════════════════════════╗
║     ✅ <b>ＲＧＸ ＨＯＳＴＩＮＧ: NODE.JS RUNNING</b> ✅  ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
║ 🆔 <b>PID:</b> {process.pid}
║ ⏱️ <b>Started:</b> {datetime.now().strftime('%H:%M:%S')}
...
"""
            ...
            error_msg = f"""
╔══════════════════════════════════════╗
║     ❌ <b>ＲＧＸ ＨＯＳＴＩＮＧ: NODE.JS FAILED</b> ❌    ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
║ ❗ <b>Exit Code:</b> {process.returncode}
╠══════════════════════════════════════╣
...
def stats_command(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐆𝐚𝐭𝐡𝐞𝐫𝐢𝐧𝐠 ＲＧＸ ＨＯＳＴＩＮＧ 𝐬𝐭𝐚𝐭𝐬...", duration=2)
...
def speed_command(message):
    ...
    msg = send_spinner_animation(message.chat.id, "𝐓𝐞𝐬𝐭𝐢𝐧𝐠 ＲＧＸ ＨＯＳＴＩＮＧ 𝐬𝐩𝐞𝐞𝐝...", duration=2)
...
def running_command(message):
    """Show running bots"""
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐅𝐞𝐭𝐜𝐡𝐢𝐧𝐠 ＲＧＸ ＨＯＳＴＩＮＧ 𝐛𝐨𝐭𝐬...", duration=1)
    running_bots = []
...
def process_broadcast(message):
    ...
    if not broadcast_text:
        ...
        return
    progress_msg = bot.send_message(message.chat.id, "📢 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 ＲＧＸ ＨＯＳＴＩＮＧ 𝐛𝐫𝐨𝐚𝐝𝐜𝐚𝐬𝐭...")
    success = 0
    failed = 0
...
"""
    ...
    result_text = f"""
╔══════════════════════════════════════╗
║     ✅ <b>ＲＧＸ ＨＯＳＴＩＮＧ 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐄</b> ✅ ║
╠══════════════════════════════════════╣
║
║  📤 𝐓𝐨𝐭𝐚𝐥: {total}
║  ✅ 𝐒𝐮𝐜𝐜𝐞𝐬𝐬: {success}
...
"""
    ...
    try:
        bot.send_message(target_user, f"🎉 𝐘𝐨𝐮'𝐯𝐞 𝐛𝐞𝐞𝐧 𝐬𝐮𝐛𝐬𝐜𝐫𝐢𝐛𝐞𝐝 𝐟𝐨𝐫 {days} 𝐝𝐚𝐲𝐬 𝐛𝐲 ＲＧＸ ＨＯＳＴＩＮＧ!")
    except:
        pass
...
def handle_text(message):
    ...
    if text == "📢 Updates Channel":
        bot.send_message(message.chat.id, f"📢 𝐉𝐨𝐢𝐧 𝐨𝐮𝐫 ＲＧＸ ＨＯＳＴＩＮＧ 𝐮𝐩𝐝𝐚𝐭𝐞𝐬:\n{UPDATE_CHANNEL}")
...
def show_user_files(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐋𝐨𝐚𝐝𝐢𝐧𝐠 ＲＧＸ ＨＯＳＴＩＮＧ 𝐟𝐢𝐥𝐞𝐬...", duration=1)
    files = user_files.get(user_id, [])
    if not files:
        text = """
╔══════════════════════════════════════╗
║       📂 <b>ＲＧＸ ＨＯＳＴＩＮＧ: YOUR FILES</b> 📂   ║
╠══════════════════════════════════════╣
║
...
def stop_all_bots(call):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        ...
        return
    bot.answer_callback_query(call.id, "🛑 𝐒𝐭𝐨𝐩𝐩𝐢𝐧𝐠 𝐚𝐥𝐥 ＲＧＸ ＨＯＳＴＩＮＧ 𝐛𝐨𝐭𝐬...")
    stopped = 0
...
def show_admin_logs(call):
    ...
    try:
        ...
        conn.close()
        if logs:
            text = "📋 <b>ＲＧＸ ＨＯＳＴＩＮＧ: RECENT LOGS</b>\n"
            for log in logs:
                text += f"👤 {log[0]} | {log[1]}\n{log[2][:30]}...\n🕐 {log[3][:16]}\n"
        else:
            text = "📋 𝐍𝐨 𝐥𝐨𝐠𝐬."
...
def cleanup_on_exit():
    ...
    logger.info("𝐂𝐥𝐞𝐚𝐧𝐢𝐧𝐠 𝐮𝐩 ＲＧＸ ＨＯＳＴＩＮＧ...")
    for script_key in list(bot_scripts.keys()):
        try:
            script_info = bot_scripts[script_key]
            kill_process_tree(script_info)
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("🤖 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 ＲＧＸ ＨＯＳＴＩＮＧ 🦁 𝐁𝐨𝐭...")
    ...
    while True:
        try:
            ...
            logger.error(f"ＲＧＸ ＨＯＳＴＩＮＧ 𝐞𝐫𝐫𝐨𝐫: {e}", exc_info=True)
            time.sleep(5)
